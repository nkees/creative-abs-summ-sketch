    ...
    for i in tqdm(range(len(raw_dialogs))):
        ...
        for sent in dialog_sents:
            ...
            if sent.startswith("[") or sent.startswith("("):
                ...
                if sent == "[SCENE_BREAK]":
                    continue
                elif sent.startswith("[Scene:"):
                    sent = re.sub("Scene:", "", sent)
                    sent = sent.strip()
                cur_dialog = sent
                cur_speaker = None
            else:
                ...
                if len(split_text) == 2:
                    cur_speaker = split_text[0]
                    cur_dialog = split_text[1]
                    ...
                    if len(split_text) == 2:
                        ...
                        cur_speaker = None
                        cur_dialog = split_text[0]
            # Preprocess cur_dialog to remove emojis
...
def main():
    data_dir = '../SUMMscreen/fd/'
    dump_data_dir = '../SUMMscreen/fd/clean_data/'
