...
@labeling_function()
def lf_scene_keyword(x):
    matches = ['[', ']', '(', ' )', ]
    not_matches = [') ', ' (']
    # return WHY if any(item in x.text.lower() for item in matches) else ABSTAIN
    return SCENE if (any(item in x.text.lower() for item in matches) and
                   all(item not in x.text.lower() for item in not_matches)) else ABSTAIN
...
def get_snorkel_label(train_dialogs, eval_dialogs): #, test_dialogs):
    ...
    lfs = [lf_why_keyword, lf_what_keyword, lf_where_keyword, lf_when_keyword, lf_confirm_keyword, lf_scene_keyword]
    applier = PandasLFApplier(lfs)
    L_train = applier.apply(train_data)

    # Train a model
    label_model = LabelModel(cardinality=7, verbose=True)
    label_model_init = copy.deepcopy(label_model)
